...
## Operational lesson (locked by Joyce 2026-07-14)

**Phase 1C and all future long API runs go on Sherlock (or another always-on box), NOT the laptop.** Phase 1B ran locally and lost ~20 h of wall time to laptop realities: lid-close sleeps, 7+ Wi-Fi outages across home/campus, and Stanford's network throttling the 96-thread parallel pattern (bot-like traffic; hotspot was 35x faster). Compute is irrelevant (the job is API-wait-bound) — what Sherlock buys is uptime and stable networking. Setup notes: keys via `chmod 600` env files (never git), GSS extract (~2 GB) must be copied over, tmux + the driver's `--parallel 8 --parallel-respondents 12` works as-is; keep qwen's `_ModelPacer` (RPM caps are account-level and follow you anywhere).

## What changed recently

- **2026-07-13**: Driver gained wall-clock-only parallelism for the Phase 1B run: `--parallel` (thread pool over the within-respondent calls) + `--parallel-respondents` (window of respondents in flight; primary-only runs). Verified byte-identical records vs sequential under a stubbed router (incl. resume no-op + partial resume). Root cause of the slow run: OpenRouter caps qwen3-max at 20 rpm account-wide ("high demand") — `_MODEL_CONCURRENCY_CAP` throttles qwen to 3 in-flight calls so retries don't exhaust into persisted `provider_error` records (that happened to rids 12/14 in the sequential run; their 6 records were purged 2026-07-13 and redone via resume). The qwen arm floors Phase 1B wall time at ~33 h unless the cap lifts.
