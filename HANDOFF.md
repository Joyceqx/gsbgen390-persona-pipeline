...
GSBGEN390 is a Stanford GSB master's thesis on **feature attribution for LLM persona synthesis** — when an LLM is prompted to respond as a specific human individual, which input feature categories (demographic / behavioral / psychological / attitudinal) actually drive prediction quality, and how does this contribution vary across outcome dimensions (attitudes / personality / behavior)? Phase 1 answers this question for the **attitude** outcome dimension at N≈1500 using GSS 2024 public data, a multi-model OpenRouter panel + GPT-4o anchor, R1+R2 leakage hygiene (battery exclusion + regression-baseline comparator), and the §12.2 quality-primary model-selection rule. Phase 2 (planned) extends the analysis to **personality** (BFI-44) + **behavioral economic games** outcome dimensions via targeted Cookiy collection with 2-week recontact baseline. Park et al. 2024 (arXiv:2411.10109 v2) is the most-cited prior work in LLM persona simulation; this project anchors against Park for cross-paper benchmarking (GPT-4o anchor on N=100 subset → direct per-item raw accuracy comparison with Park v2 SI Table 3, NOT normalized Park-style headline) but the research question stands independently. Pilot phase (N=2+1 via Cookiy) is complete; **the base Phase 1 driver and audit pipeline are implemented + tested; the two co-primary Battery LOO and Shapley robustness tools are SPECIFIED but NOT yet implemented (`battery_loo.py` + `shapley_decomposition.py`). Blockers for paid runs: (a) drop OpenRouter API key into `OpenRouter_api.txt`; (b) file OSF pre-registration per §9e checklist.**
...
3. **R1 — battery exclusion**: when predicting any item in a battery, drop the entire battery from prompt (mirrors Park BFI rule; **34 batteries + 17 singletons** in `gss_battery_map.json` v0.2)
4. **R2 — regression-baseline comparator**: non-LLM regression on same input gives a comparator MAE that the LLM can be measured against; **rhetorical** decomposition LLM_MAE ≈ regression_MAE + LLM_gain, NOT a literal causal partition. "LLM gain" = empirical model-specific predictive value beyond a supervised baseline, NOT direct evidence of "persona reasoning" (`regression_baseline.py`; caveat in `gss_phase1_design.md` §9c.4)
...
## 12. The one-paragraph paper claim (revised 2026-05-09 night per cleanup audit)

*Phase 1 of this thesis is a large-N feature-attribution study for LLM persona prediction of GSS attitude outcomes (N=1500 from GSS 2024 cross-section). It investigates **which survey-collectible feature categories and construct-level batteries improve same-wave attitude prediction**, under leakage-clean and pre-registered aggregation. The design has two co-primary analyses nested at different levels of the same attribution-question family: (1) a **4-bin LOO ablation** (broad: which feature category contributes most across demographic / behavioral / psychological / attitudinal); (2) a **34-battery LOO across all 4 bins** (mechanistic: within each pre-registered bin, which construct-level batteries drive the signal). Multiplicity is controlled by **nested Holm-Bonferroni primary** within each bin family, with **joint-34 Holm sensitivity** as a gate for cross-bin claims. Bin-level Shapley decomposition serves as 4-bin robustness. Leakage hygiene is provided by **R1 battery-level structural exclusion** (per `gss_battery_map.json` v0.2: 34 batteries) and **R2 regression-baseline comparator** (a non-LLM Ridge/Logistic baseline on the same R1-respecting input pool). LLM panel: Phase 1a runs all 4 cheap OpenRouter models on N=100 + GPT-4o anchor for cross-model robustness and §12.2 quality-primary model selection; Phase 1b runs the single §12.2-selected model on N=1500. The N=1500 headline is the selected-model result; per-model and panel-synthesized results are reported as N=100 1a robustness summaries, NOT the headline. Park et al. 2024 (arXiv:2411.10109 v2) is the most-cited prior LLM-persona work and is used here only as a **per-item raw accuracy benchmarking anchor** (N=100 GPT-4o subset comparable to Park v2 SI Table 3), NOT as the project's defining framework and NOT through normalized Park-style fidelity comparison. The contribution is: (a) the first leakage-clean, preregistered, large-N hierarchical feature-attribution framework for LLM persona prediction of public-survey attitudes; (b) battery-level structural exclusion (R1) + regression comparator (R2) as transferable methodological artifacts; (c) §12.2 quality-primary multi-model selection rule with named Qwen-2.5-72B fallback as a reusable OSF template. Discussion situates findings against six candidate cognitive/sociological frameworks (per `theory_interpretation_guide.md`) qualitatively, NOT as a horse race; null or mixed theoretical alignment is reported with equal prominence to positive alignment. Phase 1 alone is a strong empirical/methodological paper but does NOT test general human-simulation fidelity — the higher-impact thesis is Phase 1 + Phase 2 together (attitude / personality / behavioral-game outcome-stratified attribution).*
