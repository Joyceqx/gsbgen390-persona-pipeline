    ...
    for cond_name, drop_bin in conditions:
        ...
        for item in primary_eval_items:
            ...
            if truth is None:
                ...
                continue
            # R1: battery exclusion per item (or empty set if disabled via
            # --no-r1-exclusion for Park v2-exact anchor comparison).
            if no_r1_exclusion:
                excludes = set()
            else:
        ...
        if do_primary:
            ...
            if need_primary:
                records = run_primary_one_respondent(
                    ...
                    no_r1_exclusion=no_r1_exclusion,
                )
...
def _cli():
    ...
    p.add_argument("--no-r1-exclusion", action="store_true",
                   help="Disable R1 battery exclusion (Park v2 protocol). When set, "
                        "the persona includes all GSS variables in the same battery as "
                        "the eval item being predicted — this leaks within-battery "
                        "correlation into the prediction. Use ONLY for direct Park v2 "
                        "comparability runs; main analysis keeps R1 enabled.")
...
if __name__ == "__main__":
    ...
    if args.phase1a:
        ...
        n_samples = 2
        do_primary = True
        # Locked Option A 2026-05-10: full anchor runs sensitivity_eval too (for
        # Park v2 SI Table 3 comparability). Now overridable by --primary-only
        # so Joyce can run a 12-item-only "simplified" anchor for direct
        # comparison with cheap panel headline numbers without paying for the
        # 118 extra sensitivity items.
        do_sensitivity = not args.primary_only
        r1_status = "OFF (Park-exact)" if args.no_r1_exclusion else "ON (our protocol)"
        items_scope = "primary_eval (12 items)" if args.primary_only else "primary + sensitivity (130 items)"
        print(
            f"  [mode=--phase1b-anchor] N=100 selection-split subset, GPT-4o, "
            f"n_samples=2, R1 battery exclusion = {r1_status}, items = {items_scope}.",
            file=sys.stderr,
        )
    elif args.smoke:
    ...
    for pid in prompt_ids:
        ...
        run_phase1(
            ...
            conditions=active_conditions,
            no_r1_exclusion=args.no_r1_exclusion,
        )
