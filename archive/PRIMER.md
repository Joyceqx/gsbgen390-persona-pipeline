...
The thesis is structured as **two complementary phases** producing a 4 (feature category) × 3 (outcome dimension) feature-importance matrix in one semester:
- **Phase 1 — GSS public-data** (N≈1,500 GSS Three-Wave Panel; covers attitudinal row at Park-comparable N with within-person retest baseline)
- **Phase 2 — Interview-decomposed study** (N=20-30 Prolific respondents, 30-45 min modular AVP-style interview with 4 content modules ↔ 4 feature bins, 2-week separation, BFI-44 + game vignettes + GSS held-out outcomes; LOO ablates **at the interview-content level** — the actual decomposition Park's monolithic "interview" condition needs)
