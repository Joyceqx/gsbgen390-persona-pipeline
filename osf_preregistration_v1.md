...
**Companion repo**: https://github.com/Joyceqx/gsbgen390-persona-pipeline (commit hash to be filled in at OSF lock time after the final pre-lock commit lands; the 2026-05-09 evening draft commit `b5a9779` is superseded by Audit-2/3/fresh/fresh-2/fresh-3/fresh-4/fresh-5 fixes landed 2026-05-09 night through 2026-05-10 night)
...
5. **Driver runtime extension timing**: when do the `gss_driver.py` orchestration extensions for Shapley + Battery LOO get implemented? (Wording corrected 2026-05-10 per Audit-fresh-2 polish review — Battery LOO does NOT consume Phase 1b records; Phase 1c runs **new** `condition="battery_loo_drop_<name>"` records on the §12.2-selected model at N=3,309. Phase 1b headline existing only justifies the Phase 1c spend.)
   - **Default**: Phase 1a-time orchestration work for Shapley (Shapley extends the Phase 1a 5-condition panel to the full 16-condition enumeration on the same N=200); Phase 1c-time orchestration work for Battery LOO (Phase 1c runs new `battery_loo_drop_<name>` conditions on the §12.2-selected model at N=3,309 — analyzer is `battery_loo.py`, currently a `--battery-loo` NOT-IMPLEMENTED stub in `gss_driver.py`).
   - Joyce-call: deviate from default if needed.

6. **Bayati final signoff**: faculty advisor approval on the OSF v1 draft as a whole.

7. **Phase 1 budget approval (~$756 total, locked 2026-05-10 per Joyce decision Option A; supersedes the 2026-05-09 ~$875 estimate which had cheap panel running sensitivity)**: the current total is ~$756, broken down as ~$237 (core: smoke + 1a cheap N=200 + 1b cheap N=3,309 + GPT-4o anchor with sensitivity) + ~$481 (Battery LOO co-primary at N=3,309) + ~$38 (Shapley 16-condition extension on N=200 1a panel; arithmetic: 11 multi-bin LOO conditions × 12 items × 200 respondents × 4 cheap models ≈ 105,600 calls × ~$0.000356/call, since the 5 Full+single-bin conditions are already in 1a output). Evolution: ~$280-300 (early, wrong Battery LOO grid) → ~$450 (Codex-N9-fixed at N=1,500, all-China panel) → ~$875 (Audit-3 + Joyce decisions: full N=3,309 + 100/100 split + Llama swap, **cheap panel + anchor both running sensitivity**) → **~$756 (Joyce decision Option A: cheap panel primary-only, sensitivity anchor-only per OSF §3.2 — saves ~$120 net by removing ~$173 cheap-panel sensitivity calls and adding ~$50-100 anchor sensitivity calls)**. Joyce + Bayati must approve the revised budget before Phase 1c launches; reduction options if needed: (a) Battery LOO at N=1,500 subsample (saves ~$263), (b) Battery LOO restricted to attitudinal-bin batteries only (15 of 34 → saves ~$209), or (c) defer Battery LOO to Phase 1d after Phase 1b headline justifies the spend.
