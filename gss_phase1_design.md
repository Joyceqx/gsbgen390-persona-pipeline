...
- [ ] **Decisions log appendix**: `osf_preregistration_v1.md` §16 (live, locked decisions); historical decision-evidence trails in `archive/PROJECT_SYNTHESIS.md` §4 (banner'd partially superseded)

> ⚠️ **OSF copy-source rule (locked 2026-05-09 night; paths updated 2026-05-13 after stale-doc consolidation)**: when drafting the OSF preregistration, copy text ONLY from current live-design sections of `gss_phase1_design.md` (§1.0, §4, §8.8, §8.9, §9c, §9e, §10, §11.1, §12.2, §13.0–§13.2) and from `archive/PROJECT_SYNTHESIS.md` §3 + §5 (which were aligned to the live design at the time of the 2026-05-09 night audit). Do **NOT** copy from any "What changed YYYY-MM-DD" changelog block in `archive/STATUS.md`, the top-of-file revision-note paragraph in `archive/PROJECT_SYNTHESIS.md`, or the `archive/*.SUPERSEDED-*.md` files — those preserve historical wording (e.g., "attitudinal-bin Battery LOO", "panel median primary headline", "v0.1 15 batteries", "conditional on attitudinal dominance") that contradicts the live spec and would confuse OSF reviewers.
