...
> **Live design summary (current as of 2026-05-09 night, after multiple revisions)**: Phase 1 has **two co-primary analyses** — (1) 4-bin LOO ablation (broad feature-category attribution) and (2) **34-battery LOO across all 4 bins, unconditional, nested Holm primary + joint-34 Holm sensitivity** (mechanistic cluster-level attribution). Bin-level Shapley decomposition is robustness on the 4-bin LOO ranking only. Theory framing enters Discussion section only — no horse race, no theory-bin LOO, no Stage 3 refinement. RSA / permutation importance / Friedman's H deferred to future work. Practical-effect-size thresholds (small <0.02 / modest 0.02-0.05 / substantive ≥0.05) gate substantive interpretation. Implementation status: base pipeline + audit primitives + §12.2 selector + R2 baseline are implemented and self-tested; Battery LOO + Shapley tools are *specified but not yet implemented*. See `gss_phase1_design.md` §1.0 / §8.8 / §8.9 / §13.0 / §13.2 / §9e-g for the canonical live spec.
>
> *Decision-history detail* (the path that led here, preserved for OSF decision-log purposes; see also `STATUS.md` "What changed YYYY-MM-DD" entries): six-theory horse race → lean lock → Battery LOO promoted to unconditional co-primary → battery map v0.1 (15 attitudinal) → v0.2 (34 across all 4 bins) → joint-34 sensitivity + practical-effect thresholds + hierarchical justification + estimand caveats added. Per the OSF copy-source rule (`gss_phase1_design.md` §9e final paragraph), copy ONLY from the canonical live spec when drafting OSF text — do NOT copy from this decision-history paragraph.
